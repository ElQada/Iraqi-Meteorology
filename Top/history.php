<div class="ui container center aligned" id="Top">
    <div class="ui segment Station_Information">
        ...
        <div class="Information_Item">
            ...
            <div class="ui input">
                <select id="Station" class="Select" name="Station" onchange="{ timech('history'); STtName('history'); }" style="min-width: 220px;">
                    <option disabled> اســم الـمـحـطـة</option>
                    <?php if (($_SESSION['per'] === 'admin')) { ?>
                        <option value="*"> جـمـيـع الـمـحـطـات</option>
                    <?php } ?>
                    <?php Stations('history'); ?>
                </select>
            </div>
        </div>
        <div class="Information_Item">
            <label for="date"> الـتـاريـخ </label>
            <input type="text" class="_TOP_" placeholder="Date" name="date" id="date" value="<?=_REQUEST_('date',$_SESSION['SelectDate'])?>" style="width: 110px;" onchange="{timech('history'); STtName('history');}" readonly/>
        </div>
        <div class="Information_Item">
