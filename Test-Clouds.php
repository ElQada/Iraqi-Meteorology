<?php
$Title = "أطــلــس الـغــيــوم";
$CurrentFile = "Test-Clouds";
require_once "configuration/Header.php";

$Keys = ["NS1","C1","HSHS1","NS2","C2","HSHS2","NS3","C3","HSHS3"];
$limit = 200;
if (_REQUEST_("ID"))
{
    if (_REQUEST_('Delete')&&_REQUEST_("Edit"))
    {
        $Value = MySqlX("SELECT * FROM `test_clouds` WHERE `ID` = ? LIMIT 1", [_REQUEST_("ID")], 0);
        if (MySqlX("DELETE FROM `test_clouds` WHERE `ID` = ? LIMIT 1",[_REQUEST_("ID")],'R'))
        {
            Success( 'تــم الحــذف بـنـجـاح' );
            SetRecord('0', 'DELETE', 'test_clouds', _REQUEST_("ID"),json_encode($Value,256));
        }
        else
        {
    ...
    elseif (_REQUEST_('Update')&&!_REQUEST_("Edit"))
    {
        ...
        foreach (["N","H","NH","CL","CM","CH","NS1","C1","HSHS1","NS2","C2","HSHS2","NS3","C3","HSHS3","ID"] as $Input)
        {
            $Execute[] = _REQUEST_($Input);
        }

        if (MySqlX("UPDATE `test_clouds` SET `N` = ? ,`H` = ? ,`NH` = ? ,`CL` = ? ,`CM` = ? ,`CH` = ? ,`NS1` = ? ,`C1` = ? ,`HSHS1` = ? ,`NS2` = ? ,`C2` = ? ,`HSHS2` = ? ,`NS3` = ? ,`C3` = ? ,`HSHS3` = ? WHERE `ID` = ? LIMIT 1",$Execute,'R'))
        {
            Success( 'تــم الـتـعـديـل بـنـجـاح' );
            SetRecord( '0', 'UPDATE', 'test_clouds',_REQUEST_("ID"),'');
        }
        else
        {
...
elseif (_REQUEST_("Add"))
{
    ...
    foreach (["N","H","NH","CL","CM","CH","NS1","C1","HSHS1","NS2","C2","HSHS2","NS3","C3","HSHS3"] as $Input)
    {
        $Execute[] = _REQUEST_($Input);
    }

    if ($ID = MySqlX("INSERT INTO `test_clouds` (`N`,`H`,`NH`,`CL`,`CM`,`CH`,`NS1`,`C1`,`HSHS1`,`NS2`,`C2`,`HSHS2`,`NS3`,`C3`,`HSHS3`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",$Execute,'K'))
    {
...
<form>
    ...
    <div class="ui container center aligned" style="width: 100%;">
        <div class="ui segment">
            <div class="Title"> أطــلــس الـغــيــوم </div>
            <div class="ui Over">
    ...
    <script>
        ...
        var DataTable = {
            $ID : 'Data-Table-Test-Clouds',
            ID : 'Data-Table-Test-Clouds',
            Inputs : <?=json_encode($_REQUEST)?>,
            Element : "Data-Table",
            Keys : ["Index","ID","N","H","NH","CL","CM","CH","NS1","C1","HSHS1","NS2","C2","HSHS2","NS3","C3","HSHS3","Edit"],
            $Data : <?=json_encode(MySqlX("SELECT '' as `Index`,`ID`,`N`,`H`,`NH`,`CL`,`CM`,`CH`,`NS1`,`C1`,`HSHS1`,`NS2`,`C2`,`HSHS2`,`NS3`,`C3`,`HSHS3`,'' as `Edit` FROM `test_clouds`",[],11))?>,
            Data : [],
            Name : "Data-Table-Test-Clouds",
            Table : null,
            HeadRow : '',
            Titles : [],
            Clear : function (){
                ...
                [
                    {
                        ...
                        ['NH','CL','CM','CH','NS1','C1','NS3','C2','NS3','C3'].forEach(($Input)=>{
                            InputFormating($Input,1,0,0,null,null,null,null,(ID,$Error,$Message)=>{
                                DataTable.Search(DataTable.Keys.indexOf(ID),GetElementValue(ID));
                            },(ID,Value,Message)=>{
                                if (Value === '/' || Value === '')
                                {
                        ...
                        ['HSHS1','HSHS2','HSHS3'].forEach(($Input)=>{
                            InputFormating($Input,2,0,0,null,null,null,null,(ID,$Error,$Message)=>{
                                DataTable.Search(DataTable.Keys.indexOf(ID),GetElementValue(ID));
                            },(ID,Value,Message)=>{
                        ...
                        InputFormating('N',1,0,0,null,null,null,null,(ID,$Error,$Message)=>{
                            let Value = GetElementValue(ID);
                            if (Value !== '')
                            {
                                Value = VarID(ID);
                                if (!Value) {
                                    ReadOnly(["NH","CL","CM","CH","NS1","C1","HSHS1","NS2","C2","HSHS2","NS3","C3","HSHS3"], true);
                                    SetElementValue(["NH","CL","CM","CH","NS1","C1","HSHS1","NS2","C2","HSHS2","NS3","C3","HSHS3"],'');
                                }
                                else if (Value <= 2 ) {
                                    ReadOnly(["NS2","C2","HSHS2","NS3","C3","HSHS3"], true);
                                    SetElementValue(["NS2","C2","HSHS2","NS3","C3","HSHS3"], '');
                                    ReadOnly(["NH","CL","CM","CH","NS1","C1","HSHS1"], false);
                                } else if (Value > 2 && Value <= 9) {

                                    ReadOnly(["NH","CL","CM","CH","NS1","C1","HSHS1","NS2","C2","HSHS2","NS3","C3","HSHS3"], false);
                                }
                                if (GetElementValue('H') === '/' && Value !== 9)
                                {
                            ...
                            DataTable.Search(DataTable.Keys.indexOf(ID),Value);
                        },(ID,Value,Message)=>{
                            if (Value === '')
                            {
                                return false;
                        ...
                        InputFormating('H',1,0,0,null,null,null,null,(ID,$Error,$Message)=>{
                            let Value = GetElementValue(ID);
                            if (Value !== '')
                            {
                                if (GetElementValue('H') === '/' && (VarID('N') !== 9 && GetElementValue('N') !== ''))
                                {
                                    Validation(true,'N',"( H = / )"+"فى حالة ان قيمة الــ"+" ( N = 9 )"+"قيمة حقل الـ");
                                }
                            }
