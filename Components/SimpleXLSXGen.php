...
namespace Shuchkin;
require_once "EditSimpleXLSXGen.php";
/**
 * Class SimpleXLSXGen
...
<Override PartName="/xl/workbook.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet.main+xml"/>'
    ...
    {
        return $this->saveAs(($this->title ?: date('YmdHi')) . '.xlsx');
    }
    ...
    {
        return $this->downloadAs(($this->title ?: date('YmdHi')) . '.xlsx');
    }
    ...
    protected function _write($fh)
    {
        ...
        foreach ($this->template as $cfilename => $template) {
            if ($cfilename === 'xl/_rels/workbook.xml.rels') {
                ...
                $replace = [date('Y-m-d\TH:i:s\Z'), self::esc($this->author), self::esc($this->title), self::esc($this->subject), self::esc($this->keywords), self::esc($this->description), self::esc($this->category), self::esc($this->language), self::esc($this->lastModifiedBy)];
                $template = str_replace($search, $replace, $template);
                $this->_writeEntry($fh, $cdrec, $cfilename, $template);
                $entries++;
