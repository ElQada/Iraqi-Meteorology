...
if (isset($_POST['AccountCode'], $_POST['image']))
{
    if (strlen($_POST['image'])>25)
    {
        ...
        $RecordID = SetRecord(null, 'LOGIN', 'accounts','0',json_encode(['NameOrCode'=>$_POST['AccountCode'],'File'=>$imageData]),null,$_POST['AccountCode'].'@LOGIN@'.date('Y-m-d'));
        header('Location: ../courier.php');
        exit();
    }
    else
...
<head>
    <meta charset="utf-8">
    <title>تسجيل دخـــول ( حـضـور )</title>
...
<div class="login">
    ...
    <center> <h2>  تسجيل دخـــول ( حـضـور ) </h2> </center>
    ...
    <form method="post" autocomplete="off" onsubmit="{captureImage();}">
        ...
        <input type="submit" value="تسجيل دخـــول ( حـضـور )">
    </form>
    <hr>
    <center>
